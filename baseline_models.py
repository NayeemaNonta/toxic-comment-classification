...
# best_model = scores_df.sort_values('macro_f1', ascending=False).iloc[0]['model']
...
def train_and_evaluate_baselines(X_text, y, output_dir="results", SEED=42):
    ...
    for name, model in baselines.items():
        ...
        # Save report
        report = classification_report(y, y_pred, target_names=y.columns, zero_division=0)
        with open(f"{output_dir}/reports/classification_report_{name}.txt", "w") as f:
            f.write(f"=== Baseline: {name} ===\n\n")
            f.write(report)
    ...
    best_model = scores_df.sort_values(by="macro_f1", ascending=False).iloc[0]['model']
